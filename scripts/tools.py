...
def calculate_broadening_score(title: str, abstract: str) -> float:
    ...
    base_matches = sum(1.0 for kw in RESEARCH_KEYWORDS_BASE if kw.lower() in text)
...
def search_broadening_papers(focus_area: str = "", max_results: int = 6) -> List[Dict]:
    ...
    if focus_area:
        query = (
            'deep hedging OR "interest rate" OR CVA OR "portfolio optimization" '
            f"{focus_area} "
            "(extension OR survey OR review OR hybrid OR future OR emerging OR beyond)"
        )
    else:
        query = (
            'deep hedging (survey OR review OR extension OR "future directions" '
            'OR reinforcement OR "multi-asset" OR quantum OR "graph neural" '
            'OR "stochastic control" OR hybrid OR generative)'
        )
            ...
            results.append(
                {
                    ...
                    "why_broaden": (
                        "可提供新理论框架 / 新应用场景 / 跨领域方法 / 未来方向启发"
                        if broadening_score > 0.6
                        else "潜在扩展点，值得精读"
                    ),
                }
            )
...
def download_paper_by_title(title: str) -> str:
    ...
    try:
        ...
        return f"自动下载失败，请手动打开 Scholar 链接：{pub.get('url')}"
...
if __name__ == "__main__":
    print("Deep Hedging Broadening Skill loaded.")
